...
let sentPassword = function(email, name, url) {
  ...
  nodemailer.createTestAccount((err, account) => {
    ...
    let mailOptions = {
      ...
      html: '<p>Bonjour ' + name + ' <br> Afin de vous modifier votre mot de passe, nous vous invitions à cliquer sur le lien suivant : <a href=' + url + ' target="_blank">Régénérez votre mot de passe </a> </p> '
    };
    ...
    if (!user) {
      ...
      User.update({
        ...
        if (error) return res.send(error);
          ...
          res.send({
            success: 'Un mail vient de vous être envoyé, merci de verifier votre boite et suivre les instructions pour modifier votre mot de passe.'
          });
        }
      })
